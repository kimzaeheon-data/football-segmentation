...
YOLOv8 Segmentation은 적은 데이터 환경에서도 안정적인 객체 분할 성능을 보였고, U-Net은 데이터 부족과 클래스 불균형의 영향을 더 크게 받았습니다.

## 정성적 비교

<p align="center">
  <img src="data_sample/다운로드.png" width="800"/>
</p>
- 왼쪽부터 Input, Ground Truth, U-Net, YOLO 결과

![Result](data_sample/다운로드.png)
...
**pretrained 기반 instance segmentation 모델(YOLO Segmentation)** 이 더 안정적인 성능을 보였습니다.

즉, 모델 성능은 아키텍처 자체보다도  
**데이터 규모, 사전학습 여부, 문제 구조**에 크게 영향을 받는다는 점을 확인했습니다.
...
pretrained 기반의 instance segmentation 모델(YOLO Seg)이 더 안정적인 성능을 보였습니다다.
...
본 프로젝트를 통해 semantic segmentation과 instance segmentation의 차이를 실제 데이터셋에서 비교할 수 있었습니다.

특히,  
**작은 데이터셋에서는 YOLO Segmentation이 더 강한 baseline이 될 수 있으며**,  
U-Net은 더 많은 데이터와 pretrained encoder가 결합될 때 성능 향상 가능성이 크다는 점을 확인했습니다.
